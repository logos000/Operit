...
const BilibiliAssistant = (function () {
    ...
    async function ensureActivity(activityName: string = "", packageName: string = BILIBILI_PACKAGE, enterActivity: () => Promise<boolean> = async () => true, tryMax: number = 1): Promise<boolean> {
    ...
    async function search_video(params: { keyword: string, filter_type?: string }): Promise<any> {
    ...
    async function applySearchFilter(filterType: string): Promise<void> {
        try {
            ...
            let filterButton: UINode | undefined = undefined;
    ...
    async function getSearchResults(): Promise<any[]> {
        try {
            const page = await UINode.getCurrentPage();
            const results: any[] = [];
    ...
    async function play_video(params: { video_title?: string, video_index?: number }): Promise<any> {
        ...
        try {
            const page = await UINode.getCurrentPage();
            let targetVideo: UINode | undefined = undefined;
    ...
    async function wrap_bool(func: (params: any) => Promise<any>, params: any, successMessage: string, failMessage: string, additionalMessage: string = "") {
        try {
