...
const various_search = (function () {
    ...
    function convertParamTypes(params, paramTypes) {
        ...
        // {} 创建一个空对象字面量
        const result = {}; // 创建一个新的对象来存储转换后的结果
    ...
    async function search_wrap(func, // 函数类型定义
    params // 参数可以是任意类型
    ) {
        // const 声明常量
        // `` 模板字符串
        ...
        catch (error) { // catch 捕获异常，error: any 类型注解
    ...
    async function combined_search(params) {
        ...
        // [] 数组字面量，创建空数组
        const validPlatforms = []; // 创建数组来存储有效的平台
        const invalidPlatformNames = []; // 无效平台名称
        const invalidPlatformErrors = []; // 对应的错误信息
        ...
        const searchPromises = validPlatforms.map(async (platform) => {
            ...
            // platform 添加新属性
            return Object.assign(Object.assign({}, result), { platform }); // 在结果中添加平台信息
        });
